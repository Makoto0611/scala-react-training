...
要件：
```
- テキストボックスに文字を入力すると、コンテンツIDに含まれるものだけ表示される
- テキストボックスが空のときは全件表示
...
フィルター結果が0件のとき「該当するコンテンツがありません」を表示する。
条件付きレンダリングを3通りの書き方（三項・&&・if）で実装し、
自分が読みやすいと思う書き方に統一する。

### Part3：選択状態を管理する

各 ContentCard をクリックすると選択状態になり、背景色が変わる。

要件：
```
- 別の ContentCard をクリックすると選択が移る
- 同じ ContentCard を再クリックすると選択が解除される
- 選択中の contentId を useState で管理する（selectedContentId: string | null）
```

### Part4：ソート機能を追加する

「視聴数」「いいね数」「いいね率」のいずれかでソートできるようにする。
...
フィルター文字列と選択状態は、ContentCard の親コンポーネントで持つ必要がある。
